...
8. **生成两个文件**（单文件策略，多对象追加而非拆分）：
   - `ontology-config.yaml` - 技术配置文件（所有核心对象完整配置 + 关联对象骨架，统一在一个文件中）
   - `ontology-documentation.md` - 业务说明文档（所有核心对象的文档统一在一个文件中，按对象分大章节，顶部有目录导航）
   - **多对象场景**：当为第2个及以后的对象建模时，将新对象的 YAML 配置追加到已有的 `ontology-config.yaml`（替换对应骨架），将新对象的文档追加到已有的 `ontology-documentation.md`（新增章节）。**绝不为每个对象创建单独的文档文件**——YAML 和文档必须保持一致的单文件策略，方便统一查阅和交叉引用校验
...
**完整的常见问题列表**（28个陷阱）：见 `references/gotchas.md`（其中 1-17 为单对象建模陷阱，18-24 为多对象大规模建模心得，25-28 为本体扩展心得）
